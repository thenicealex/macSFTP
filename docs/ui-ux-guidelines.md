...
- 本项目是支持多窗口和多标签的 macOS 桌面应用，使用 GPUI 自绘 UI。
- 每个窗口的核心界面是：顶部 tab bar、左右文件 pane、底部 transfer drawer、底部 status bar。
...
- **窗口内上下文完整**：用户应在一个工作窗口内完成连接、浏览、传输、冲突处理和错误恢复；打开第二窗口不应成为完成基本流程的前提。
...
- 是否保持单个窗口内的工作上下文完整，并且没有破坏多窗口间的状态隔离？
