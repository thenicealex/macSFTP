...
- 分支合并或清理前，先一次性盘点本地/远程分支、未推送提交、目标分支保护规则和允许的合并方式；不要靠连续失败来探测仓库策略。
- 受保护分支必须走 PR 时，PR 创建后立即同时检查 required checks、未解决 review threads 和其他合并门禁。禁止等 CI 全部结束后才发现早已存在的评审阻塞。
- 一轮检查发现的必需修复应尽量集中为一次更新后再推送；每次 push 都会重跑 CI，不要把可同时处理的问题拆成多轮验证。
- CI 或评审暴露出超出原任务范围的代码、依赖或产品行为修改时，先说明阻塞及最小选项；未经用户确认，不要为了完成 Git 操作自动扩大实现范围。
- 等待远程检查时使用有界等待并只报告状态变化；禁止高频轮询和重复播报相同的 pending 状态。
- 删除分支前必须验证需要保留的提交已能从目标分支到达，并使用明确列出的 ref 执行删除；删除后同时核对本地和远程 refs。相关的只读检查和同阶段安全操作应合并执行，减少无意义的往返与授权请求。
