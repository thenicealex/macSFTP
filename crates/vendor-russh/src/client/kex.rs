...
#[cfg(feature = "rsa-sha2-host-key")]
use {
    aws_lc_rs::signature::{
        RSA_PKCS1_1024_8192_SHA256_FOR_LEGACY_USE_ONLY,
        RSA_PKCS1_1024_8192_SHA512_FOR_LEGACY_USE_ONLY, RsaPublicKeyComponents,
    },
...
fn verify_server_signature(
    ...
    #[cfg(feature = "rsa-sha2-host-key")]
    if let Algorithm::Rsa { hash } = server_signature.algorithm() {
        let Some(public_key) = server_host_key.key_data().rsa() else {
            return false;
        };
        let (Some(modulus), Some(exponent)) = (
            public_key.n().as_positive_bytes(),
            public_key.e().as_positive_bytes(),
        ) else {
            return false;
        };
        let parameters = match hash {
            Some(HashAlg::Sha256) => &RSA_PKCS1_1024_8192_SHA256_FOR_LEGACY_USE_ONLY,
            Some(HashAlg::Sha512) => &RSA_PKCS1_1024_8192_SHA512_FOR_LEGACY_USE_ONLY,
            _ => return false,
        };
        ...
        .verify(parameters, exchange_hash, server_signature.as_bytes())
        .is_ok();
    }

    signature::Verifier::verify(server_host_key, exchange_hash, server_signature).is_ok()
}
    ...
    pub fn step(
        ...
        match self.state {
            ...
            ClientKexState::WaitingForDhReply { mut names, mut kex } => {
                ...
                if !verify_server_signature(&server_host_key, hash.as_ref(), &signature) {
...
#[cfg(all(test, feature = "rsa-sha2-host-key"))]
mod tests {
    use data_encoding::BASE64;
    use ssh_key::{Algorithm, HashAlg, PublicKey, Signature};

    use super::verify_server_signature;

    const RSA_PUBLIC_KEY: &str = "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAgQDRlWNDvO+ijXnvpTOKXqqFDPe2SdQjjo2INk7DrpiRlhr0x4xGtl9prDIy/ETQfnT/a6W6/ljLyZNGQPqei6bvnUXq9iYfQM0O0miFYREV8fo0J6oEI++Tz3iuwVVWb6LKggTNeNT+h1rx0Rb9fG/YBTjVrt+7/bmU4OI3U47gXQ==";
    const RSA_SHA512_SIGNATURE: &str = "MqxWwTOHFDNru2WmF2EqJojs6Q54zYfRLJSqVDke5r/fTEMiPPtRZWyt0Rj01U8WSpTKZiNX3mjb8+K7q6X99jRzxoYu9maFrloJb8/J7PKYeavRZYLWCE4QHBsVDVV3vWgguQlzfopWliTo2p/ICAHdEsjo1Y3oBu3Pe8Gl8D0=";

    #[test]
    fn verifies_legacy_rsa1024_sha512_host_signature_with_aws_lc() {
        let public_key = PublicKey::from_openssh(RSA_PUBLIC_KEY).unwrap();
        let signature_bytes = BASE64.decode(RSA_SHA512_SIGNATURE.as_bytes()).unwrap();
        ...
        assert!(verify_server_signature(
            &public_key,
            b"macSFTP RSA-1024 host-key verification test",
            &signature,
        ));
        assert!(!verify_server_signature(
            &public_key,
            b"tampered exchange hash",
            &signature,
        ));
    }
}
